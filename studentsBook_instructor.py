...
class student_Booking(object):
    def __init__(self, window):
        ...
        self.txt_title = Label(self.Top, width=40, font=('arial', 24), text="Cource Management - DanceFeet System",
                               borderwidth=0, relief="groove", fg='#01579b')
        ...
        # self.btn_exit = Button(self.Buttons, width=10, text="Exit", command=self.Exit)
        ...
        "id", "firstname", "surname", "email", "gender", "dob", "TP", "style", "hrate", "instructor", "availability",
        "address"), selectmode="extended", height=500, yscrollcommand=self.scrollbary.set,
    ...
    def Update(self):
        ...
        self.tree.delete(*self.tree.get_children())
        self.cursor.execute(
            f"UPDATE `students` SET `firstname` = '{self.firstname_entry}', `gender` = '{self.gender_entry}', `surname` = '{self.surname_entry}',  `email` = '{self.email_entry}',  `dob` = '{self.dob_entry}', `tp` = '{self.tp_entry}', `style` = '{self.style_entry}', `hrate` = '{self.hrate_entry}', `instructor` = '{self.instructor_entry}', `availability` = '{self.availability_entry}', `address` = '{self.address_entry}' WHERE `ID` = '{self.id_entry}'")
        self.conn.commit()
        self.cursor.execute("SELECT * FROM `students` ORDER BY `ID` ASC")
        self.fetch = self.cursor.fetchall()
        for self.data in self.fetch:
    ...
    def OnSelected(self, event):
        ...
        self.instructor_combo['values'] = self.combo_values_input(self.selecteditem[8], self.selecteditem[7],
    ...
    def combo_values_input(self, student_rate, student_style, student_availability):
        ...
        self.query = self.cur.execute(
            f'SELECT name FROM instructors WHERE hrate <= {student_rate} AND styles LIKE "%{student_style}%" AND availability LIKE "%{student_availability}%"')
